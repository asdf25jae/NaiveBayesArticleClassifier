...
def main():
    ...
    for key, value in sorted(liberal_dict.iteritems(), key=lambda (k,v): (v,k),
                             reverse=True):
        logOdds = math.log(value) - math.log(conserv_dict[key])
        lib_log_dict[key] = logOdds
        #print "%s %.04f" % (key, logOdds)
    for key, value in sorted(conserv_dict.iteritems(), key=lambda (k,v): (v,k),
                             reverse=True):
        logOdds = math.log(value) - math.log(liberal_dict[key])
        #print "%s %.04f" % (key, logOdds)
        cons_log_dict[key] = logOdds
    count = 0
    for key, value in sorted(lib_log_dict.iteritems(), key=lambda (k,v) : (v,k),
                             reverse=True):
        if count >= 20: break
    ...
    count = 0
    print " "
    for key, value in sorted(cons_log_dict.iteritems(), key=lambda (k,v) : (v,k),
                             reverse=True):
        if count >= 20: break
        print "%s %.04f" % (key, value)
        count += 1
