...
def remove_N_freq_words(words_list, N):
    ...
    for key, value in sorted(freq_dict.iteritems(), key=lambda (k,v): (v,k),
        ...
        count += 1
    return set(vocab_arr), removed_words


def update_words(words, N, rem_words):
    word_counter = Counter(words)
    for removed_word in rem_words:
        del word_counter[removed_word]
    return list(word_counter.elements())
...
def main():
    ...
    cons_temp, lib_temp = retrieve_words(train_files)
    vocabulary, rem_words = remove_N_freq_words(cons_temp+lib_temp, N)
    liberal_words = update_words(lib_temp, N, rem_words)
    conserv_words = update_words(cons_temp, N, rem_words)
